...
export function QuestionInput({
  ...
  return <div className={`space-y-4 ${className}`}>
      ...
      {showSuggestions && <Collapsible open={isOpen} onOpenChange={setIsOpen}>
          <CollapsibleTrigger asChild>
            <Button variant="ghost" className="w-full gap-2 text-sm text-muted-foreground hover:text-foreground bg-zinc-300 hover:bg-zinc-200 rounded-sm">
          ...
          <CollapsibleContent className="space-y-3 animate-accordion-down">
            ...
            <div className="space-y-2 py-[4px]">
              ...
              <button onClick={() => setQuestion("Would you support increased funding for our local library?")} className="w-full p-3 text-left text-sm bg-card border rounded-lg hover:bg-accent transition-colors">
                Would you support increased funding for our local library?
              </button>
              <button onClick={() => setQuestion("What programs or services would you like to see offered in our schools?")} className="w-full p-3 text-left text-sm bg-card border rounded-lg hover:bg-accent transition-colors">
